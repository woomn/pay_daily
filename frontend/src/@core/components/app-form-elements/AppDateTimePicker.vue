...
const { modelValue: _, ...inputProps } = (VInput as any).filterProps(props)
...
if ((compAttrs as any).config && (compAttrs as any).config.inline) {
  isInlinePicker.value = (compAttrs as any).config.inline
...
<template>
  <div class="app-picker-field">
    <VInput
      ...
      :hide-details="props.hideDetails as any"
      :class="[{
...
.flatpickr-calendar {
  ...
  .flatpickr-day {
    ...
    &.inRange.today {
      background: rgb(var(--v-theme-primary) 0.24) !important;
    }
    ...
    &:hover {
      border-color: transparent;
      background: rgb(var(--v-theme-on-surface) var(--v-hover-opacity));
  ...
  .flatpickr-months {
    ...
    .flatpickr-next-month {
      ...
      svg {
        block-size: 13px;
        inline-size: 13px;
        stroke: rgb(var(--v-theme-on-surface) var(--v-medium-emphasis-opacity));
      }
